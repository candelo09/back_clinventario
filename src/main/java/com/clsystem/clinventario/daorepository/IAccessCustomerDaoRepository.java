...
public interface IAccessCustomerDaoRepository extends CrudRepository<CustomerAccess, Long> {
    ...
    @Query(value = "select p.*, c.name, c.document, c.email, m.type_membership, " +
            ...
            "where c.document = :document and p.membership_end_date > now()", nativeQuery = true)
    ...
    @Query(value = "select p.*, c.name, c.document, c.email, m.type_membership, " +
            ...
            "left join customers c on c.id = p.id_customer " +
            "left join memberships m on m.id = p.id_membership " +
            "left join physical_progress pp on pp.id_customer = c.id", nativeQuery = true)
    List<CustomerAccess> findAllDocumentAccess();
}
